...
class Checkout_Form {
	...
	public function render_checkout_form($params) {
		...
		if (!$row) {
			...
			$log::write('[CQ Frontend Shortcode Display] [Whalestack_checkout id="' . $id . '"] doesn\'t exist but is embedded on your website.');
			return '<div class="coinqvest_payments_error_msg coinqvest-text-color-red">' . esc_html(__('Something is wrong with your Whalestack checkout shortcode.', 'coinqvest')) . '</div>';
		}

		// validate that button is active
		if ($row->status != 1) {
			$log = new Api\CQ_Logging_Service();
			$log::write('[CQ Frontend Shortcode Display] [Whalestack_checkout id="' . $id . '"] is not active but is embedded on your website.');
			return '<p class="coinqvest-text-color-red">' . esc_html(__('Your Whalestack checkout button is not active.', 'coinqvest')) . '</p>';
		}
                ...
                <div class="cq-grid">
                    ...
                    <div class="cq-row">
                        ...
                        <div class="cq-col-6 cq-center-xs">
                            <img class="cq-logo" src="' . esc_url($this->plugin_name_url) . 'assets/images/whalestack-logo.png" width="100px">
